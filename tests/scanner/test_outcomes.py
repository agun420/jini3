...
def _signal(entry="20.00", stop="18.00", target="24.00") -> Signal:
    return Signal(
        ticker="AAAA",
...
def test_resolve_outcome_win_when_target_touched_first():
    signal = _signal()
    bars = [
...
def test_resolve_outcome_treats_same_bar_touching_both_as_a_loss():
    signal = _signal()
