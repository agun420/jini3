...
def resolve_outcome(signal: Signal, bars: Sequence[MinuteBar]) -> SignalOutcome | None:
    ...
    A bar whose range covers both stop and target is treated as a loss --
    intrabar order can't be known from OHLC data alone, so the worse case is
    assumed rather than guessed. Returns None if unresolved through the given
    bars; the caller should either fetch more bars later or call
    `expire_at_close` once the session has ended.
    """
    for bar in sorted(bars, key=lambda item: item.timestamp):
        ...
        if hit_target:
            return SignalOutcome(
                ...
                return_pct=_return_pct(signal.entry_price, signal.target_price),
            )
    return None
