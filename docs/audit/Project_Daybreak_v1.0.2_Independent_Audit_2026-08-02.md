...
**Update, same audit pass:** while verifying this finding, the CI `security` job was observed failing on this PR's own (documentation-only) commit — `scripts/check_secrets.py`'s `--exclude-files r".*\.example$"` never matched `config/daybreak.example.toml` or `config/daybreak.paper.example.toml`, because those filenames end in `.toml`, not literally `.example`; the pattern only ever matched `config/daybreak-recorder.env.example`. This has been broken since the repository's initial commit, so the `security` CI job has never passed. Fixed here by widening the pattern to `r".*\.example(\.[^.]+)?$"` and adding narrow `# pragma: allowlist secret` annotations to the two remaining genuine false positives this audit already reviewed line-by-line: the pinned spec hash constant in `daybreak_evaluator/prompt.py` (not a secret at all) and the default local-dev DSN in `daybreak_recorder/settings.py` (the `DB-29` credential below, real but non-sensitive and already tracked for separate remediation — annotating it doesn't fix `DB-29`, it only tells the scanner this specific, already-documented instance has been reviewed). The scan-scope gap above (tests/migrations/schemas/root files never scanned) remains open.
